...
/-- **Direct No-Divergence via Constraint Mismatch (AXIOM-FREE)**:

    Uses ONLY proven lemmas, no axioms:
    1. `orbit_constraint_match`: Orbit patterns satisfy the constraint equation
    2. `constraint_mismatch_direct_at_cutoff`: For S > m and large m, constraint mismatches
    3. `allOnes_constraint_mismatch_at_cutoff`: For S = m (all-ones), constraint mismatches

    This is Option B from the proof strategy: direct constraint mismatch without orbit bounds. -/
theorem no_divergence_via_constraint_mismatch (n : ℕ) (hn_pos : 0 < n) (hn_odd : Odd n) (hn_gt1 : n > 1) :
    ¬DivergentOrbit n := by
  intro hdiv
  -- Get divergence: ∀ N, ∃ t, orbit_raw n t > N
  -- Choose N = n, so ∃ t, orbit_raw n t > n (orbit grows beyond n)

  -- Cutoff for constraint mismatch
  set m := max (max (2 * Nat.log 2 n + 9) 5) (Nat.log 2 n + 2) with hm_def
  have hm_ge1 : m ≥ max (2 * Nat.log 2 n + 9) 5 := le_max_left _ _
  have hm_ge2 : m ≥ Nat.log 2 n + 2 := le_max_right _ _

  -- The orbit pattern at step m
  ...
  -- Key: orbit pattern constraint matches n (PROVEN, no axioms)
  ...
  -- Pattern sum S = Σνᵢ where each νᵢ ≥ 1
  have hS_ge_m : Collatz.patternSum σ ≥ m := by
    have h := Collatz.valid_pattern_sum_ge_length hvalid
    rw [hlen] at h
    exact h

  -- Case split: S = m (all-ones) or S > m (non-all-ones)
  by_cases hS_eq_m : Collatz.patternSum σ = m
  · -- CASE 1: S = m (all-ones pattern)
    -- The pattern σ must be all-ones: [1, 1, ..., 1]
    have h_allOnes : σ = Collatz.allOnesPattern m := by
      have h_sum_len : Collatz.patternSum σ = σ.length := by rw [hlen, hS_eq_m]
      have h := Collatz.valid_pattern_sum_eq_length_implies_allOnes σ hvalid h_sum_len
      rw [hlen] at h
      exact h

    -- By allOnes_constraint_mismatch_at_cutoff: constraint mismatches for large m
    have h_mismatch : (n : ZMod (2^(Collatz.patternSum (Collatz.allOnesPattern m)))) ≠
        Collatz.patternConstraint (Collatz.allOnesPattern m) :=
      Collatz.allOnes_constraint_mismatch_at_cutoff n hn_pos m hm_ge2

    -- But h_match says n = constraint for σ = allOnesPattern m
    rw [h_allOnes] at h_match
    exact h_mismatch h_match

  · -- CASE 2: S > m (non-all-ones pattern)
    push_neg at hS_eq_m
    have hS_gt_m : Collatz.patternSum σ > m := by omega

    -- By constraint_mismatch_direct_at_cutoff: constraint mismatches for S > m and large m
    have h_mismatch : (n : ZMod (2^(Collatz.patternSum σ))) ≠ Collatz.patternConstraint σ :=
      Collatz.constraint_mismatch_direct_at_cutoff n hn_gt1 m hm_ge1 σ hlen hvalid hS_gt_m

    -- But h_match says n = constraint for σ
    exact h_mismatch h_match

/-- **Architecturally Clean No-Divergence via Supercriticality + Constraint Mismatch**:

    This version properly layers the proof components:

    **Layer 1 (SubcriticalCongruence)**: "When does supercriticality kick in?"
    - `eventual_supercriticality`: For n₀ > 1 odd, ∃ m₀ s.t. ∀ m ≥ m₀, 2^S ≥ 3^m

    **Layer 2 (ConstraintMismatch)**: "What happens in the supercritical regime?"
    - `constraint_mismatch_direct_at_cutoff`: For S > m and large m, constraint fails
    - `allOnes_constraint_mismatch_at_cutoff`: For S = m, constraint fails

    **This Layer (WanderingTarget)**: Glue that connects orbit → pattern → contradiction

    The key insight: these are logically separate jobs:
    1. SubcriticalCongruence answers "when supercritical?" (analytic: 2^S ≥ 3^m)
    2. ConstraintMismatch answers "no solutions in that regime" (pattern-level v₂ bounds)

    Architecture: SubcriticalCongruence does NOT call ConstraintMismatch.
                  ConstraintMismatch does NOT call SubcriticalCongruence.
                  WanderingTarget calls BOTH at the glue layer. -/
theorem no_divergence_via_supercriticality (n : ℕ) (hn_pos : 0 < n) (hn_odd : Odd n) (hn_gt1 : n > 1) :
    ¬DivergentOrbit n := by
  intro hdiv

  -- LAYER 1: SubcriticalCongruence provides eventual supercriticality bound
  -- For n > 1 odd, there exists m₀ such that ∀ m ≥ m₀, the orbit pattern is supercritical (2^S ≥ 3^m)
  obtain ⟨m₀_super, h_supercrit⟩ := SubcriticalCongruence.eventual_supercriticality n hn_gt1 hn_odd

  -- LAYER 2: ConstraintMismatch provides constraint cutoff
  -- Cutoff for constraint mismatch (pattern-level, does not depend on SubcriticalCongruence)
  let m₀_constraint := max (2 * Nat.log 2 n + 9) 5
  let m₀_allones := Nat.log 2 n + 2

  -- Combined cutoff: take m large enough for BOTH supercriticality AND constraint mismatch
  set m := max (max m₀_super m₀_constraint) m₀_allones with hm_def

  have hm_ge_super : m ≥ m₀_super := le_trans (le_max_left _ _) (le_max_left _ _)
  have hm_ge_constraint : m ≥ m₀_constraint := le_trans (le_max_right _ _) (le_max_left _ _)
  have hm_ge_allones : m ≥ m₀_allones := le_max_right _ _

  -- The orbit pattern at step m
  set σ := OrbitPatternBridge.orbitPattern n m with hσ_def

  -- Pattern properties (from OrbitPatternBridge, does not use SubcriticalCongruence)
  have hlen : σ.length = m := OrbitPatternBridge.orbitPattern_length n m
  have hvalid : Collatz.isValidPattern σ := OrbitPatternBridge.orbitPattern_valid n m hn_odd hn_pos

  -- GLUE: orbit_constraint_match connects orbit to pattern constraint
  have h_match : (n : ZMod (2^(Collatz.patternSum σ))) = Collatz.patternConstraint σ :=
    OrbitPatternBridge.orbit_constraint_match n hn_odd hn_pos m

  -- From SubcriticalCongruence: since m ≥ m₀_super, we have ¬(2^S < 3^m)
  -- i.e., 2^S ≥ 3^m (analytic supercriticality)
  have h_not_subcrit : ¬(2^(Collatz.patternSum σ) < 3^σ.length) := h_supercrit m hm_ge_super
  rw [hlen] at h_not_subcrit
  -- h_not_subcrit : ¬(2^S < 3^m), equivalently 2^S ≥ 3^m

  -- From valid pattern: S ≥ m (combinatorial supercriticality, trivial)
  have hS_ge_m : Collatz.patternSum σ ≥ m := by
    have h := Collatz.valid_pattern_sum_ge_length hvalid
    rw [hlen] at h
    exact h
  ...
  · -- CASE 1: S = m (all-ones pattern)
    ...
    -- LAYER 2: allOnes_constraint_mismatch_at_cutoff (pattern-level)
    have h_mismatch : (n : ZMod (2^(Collatz.patternSum (Collatz.allOnesPattern m)))) ≠
        Collatz.patternConstraint (Collatz.allOnesPattern m) :=
      Collatz.allOnes_constraint_mismatch_at_cutoff n hn_pos m hm_ge_allones
  ...
  · -- CASE 2: S > m (non-all-ones pattern)
    ...
    -- LAYER 2: constraint_mismatch_direct_at_cutoff (pattern-level)
    have h_mismatch : (n : ZMod (2^(Collatz.patternSum σ))) ≠ Collatz.patternConstraint σ :=
      Collatz.constraint_mismatch_direct_at_cutoff n hn_gt1 m hm_ge_constraint σ hlen hvalid hS_gt_m
