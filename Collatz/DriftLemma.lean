...
import Collatz.EqualCaseProof  -- Added: provides EqualCase.forcesNOne, etc.
import Mathlib
...
/-- Orbits cannot stay subcritical past the constraint cutoff.

    Uses orbit_constraint_match (orbit satisfies constraint) +
    allOnes_constraint_eventually_misses (constraint fails for all-ones patterns) +
    constraint_mismatch machinery (constraint fails for S > m patterns).

    This is the key lemma that bounds how long an orbit can remain subcritical. -/
lemma orbit_eventually_non_subcritical (n : ℕ) (hn : n > 1) (hn_odd : Odd n) :
    ∃ m₀, ∀ m ≥ m₀, ¬isSubcriticalNu (nuSum n m) m := by
  -- Get cutoff from all-ones constraint mismatch
  have h_allones := allOnes_constraint_eventually_misses n (by omega : 0 < n)
  obtain ⟨m₁, h_cutoff_allones⟩ := h_allones

  -- For the S > m case, we use constraint_mismatch machinery
  -- Cutoff: max(2*log₂ n + 9, 5)
  set m₂ := max (2 * Nat.log 2 n + 9) 5 with hm₂_def

  -- Use max of both cutoffs
  use max m₁ m₂
  intro m hm h_subcrit

  -- The orbit pattern at step m is σ = orbitPattern n m
  set σ := orbitPattern n m with hσ_def
  set S := Collatz.patternSum σ with hS_def

  -- Key facts about orbit pattern
  have h_len : σ.length = m := orbitPattern_length n m
  have h_sum : S = nuSum n m := orbitPattern_sum n m
  have h_valid : Collatz.isValidPattern σ := orbitPattern_valid n m hn_odd (by omega)

  -- By orbit_constraint_match: n satisfies the constraint for σ
  have h_match := orbit_constraint_match n hn_odd (by omega) m
  -- h_match : (n : ZMod (2^S)) = patternConstraint σ

  -- Valid patterns have S ≥ m (each ν ≥ 1)
  have hS_ge_m : S ≥ m := by
    have h1 : patternSum σ ≥ σ.length := valid_pattern_sum_ge_length h_valid
    rw [h_len] at h1
    exact h1

  -- Case split: S = m (all-ones) vs S > m
  by_cases hS_eq_m : S = m
  · -- Case 1: S = m means all-ones pattern
    -- From orbitPattern_nuSum_eq_m_implies_allOnes: σ = allOnesPattern m
    have h_nusum_eq_m : nuSum n m = m := by rw [← h_sum]; exact hS_eq_m
    have h_is_allones := orbitPattern_nuSum_eq_m_implies_allOnes n m hn_odd (by omega) h_nusum_eq_m
    -- Now apply allOnes_constraint_eventually_misses
    have hm₁ : m ≥ m₁ := le_max_left m₁ m₂ |>.trans hm
    have h_miss := h_cutoff_allones m hm₁
    -- h_miss says n ≠ constraint for allOnesPattern m
    -- But σ = allOnesPattern m and h_match says n = constraint for σ
    rw [h_is_allones] at h_match
    exact h_miss h_match
  · -- Case 2: S > m (non-all-ones)
    have hS_gt_m : S > m := Nat.lt_of_le_of_ne hS_ge_m (Ne.symm hS_eq_m)
    have hm₂ : m ≥ m₂ := le_max_right m₁ m₂ |>.trans hm
    -- For S > m patterns, use the direct cutoff mismatch lemma.
    have hm_cutoff : m ≥ max (2 * Nat.log 2 n + 9) 5 := by
      simpa [hm₂_def] using hm₂
    -- Derive divisibility from constraint match
    -- h_match : (n : ZMod (2^S)) = patternConstraint σ
    -- This means waveSumPattern σ + n * 3^m ≡ 0 (mod 2^S)
    have h_divides : 2^S ∣ Collatz.waveSumPattern σ + n * 3^σ.length := by
      have h_cong := Collatz.constraint_match_iff.mp h_match
      -- h_cong : (waveSumPattern σ : ZMod (2^S)) = -(n : ZMod (2^S)) * (3^σ.length)
      have h_sum_zero : ((Collatz.waveSumPattern σ + n * 3^σ.length : ℕ) : ZMod (2^S)) = 0 := by
        simp only [hσ_def] at h_cong ⊢
        push_cast
        rw [h_cong]
        push_cast
        ring
      exact (ZMod.natCast_zmod_eq_zero_iff_dvd _ _).mp h_sum_zero
    have h_ne := Collatz.constraint_mismatch_direct_at_cutoff n hn m hm_cutoff σ h_len h_valid hS_gt_m
    exact h_ne h_match
