...
/-- **COMPUTATIONAL VERIFICATION AXIOM** (Syracuse/orbit_raw version)

    For all positive integers n ≤ N_verified = 17 × 2^58, the Syracuse orbit
    (orbit_raw) is bounded and cannot be divergent.

    This is the Syracuse-iteration version of collatz_verified_below_bound.
    The two axioms are equivalent since both iteration schemes track the same
    underlying Collatz sequence - syracuse_raw just combines multiple steps. -/
axiom small_n_not_divergent_syracuse :
    ∀ n : ℕ, 0 < n → n ≤ N_verified → ¬DivergentOrbit n
