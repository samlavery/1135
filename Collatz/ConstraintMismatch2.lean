...
lemma v2_wave_plus_case3_bound {σ : List ℕ} {n₀ : ℕ}
    ...
    (hneq_equal : padicValNat 2 (1 + 3 * n₀) ≠ σ.head!) :
  ...
  --
  -- The _h_divides hypothesis is NOT used here - it's passed by callers who will
  -- use it to derive v₂(L) ≥ S, creating a contradiction with v₂(L) < S.
  simp only []
  intro hL_pos
  haveI : Fact (Nat.Prime 2) := ⟨Nat.prime_two⟩
  ...
  · -- Case: n₀ odd => wave cascade analysis bounds v₂(L) ≤ 2*log₂(n₀) + 7
    ...
    · -- Equal case: v₂(1+3n₀) = K — excluded by hypothesis hneq_equal
      push_neg at hdistinct
      -- hdistinct : padicValNat 2 (1 + 3 * n₀) = K = σ.head!
      -- hneq_equal : padicValNat 2 (1 + 3 * n₀) ≠ σ.head!
      exact absurd hdistinct hneq_equal
...
/-- **DIRECT constraint mismatch**: For any n₀ > 1, DISTINCT-CASE patterns with S > m eventually fail.
    ...
    **NOTE**: This only applies to the "distinct case" where v₂(1 + 3n₀) ≠ σ.head!.
    ...
    and those patterns DO satisfy the constraint. See Mod8Analysis.v2_equal_case_exact.
...
lemma constraint_mismatch_direct (n₀ : ℕ) (hn₀ : 1 < n₀) :
    ∃ m₀ : ℕ, ∀ m ≥ m₀, ∀ σ : List ℕ,
      ...
      isValidPattern σ →
      patternSum σ > m →
      padicValNat 2 (1 + 3 * n₀) ≠ σ.head! →  -- Distinct case only
  ...
  intro m hm σ hlen hvalid hS_gt_m hdistinct
  set S := patternSum σ with hS_def
  ...
  · -- ODD CASE: Use direct v₂ bound
    ...
    -- Use distinct case v₂ lemma (hdistinct hypothesis excludes the equal case)
...
/-- Direct version of constraint_mismatch_direct without existential.
    For m ≥ max(2*log n + 9, 5), DISTINCT-CASE patterns with S > m don't match n.
...
/-- Simple constraint mismatch for DISTINCT-CASE subcritical patterns.
    This is the version NoDivergence.lean can call without spectral infrastructure.

    For subcritical patterns in the DISTINCT case, the constraint eventually misses n₀.
...
lemma constraint_eventually_misses_simple
  ...
  · -- Non-all-ones case: S > m
    ...
    have hS_gt_m : patternSum σ > m := by
      ...
      rw [hlen] at h_eq
      exact hallones h_eq
    -- Now use constraint_mismatch_direct_at_cutoff (hdistinct from theorem hypothesis)
    exact constraint_mismatch_direct_at_cutoff n₀ hn₀ m hm σ hlen hvalid' hS_gt_m hdistinct
